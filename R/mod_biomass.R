...
mod_biomass_ui <- function(id){
  ...
  tagList(
    tabsetPanel(id = ns("lowtab3"),
                tabPanel("Plot",
                         br(),
                         downloadButton(ns('downloadPlotBiomass'), class = 'small-dl', 
                                        label = "Download Plot (png)"),
                         br(),
                         br(),
                         div(
                           inline(div(class = 'param-label', HTML("y-axis"))),
                           inline(selectInput(ns('yBiomass'), label = NULL, 
                ...
                tabPanel("Table",
                         br(),
                         downloadButton(ns('downloadTableBiomass'), class = 'small-dl', 
...
mod_biomass_server <- function(input, output, session, params){
  ...
  output$downloadPlotBiomass <- downloadHandler(
    filename = function() {"ypr_biomass.png"},
  ...
  output$downloadTableYield <- downloadHandler(
    filename = function() {"ypr_biomass.csv"},
    content = function(file) {
